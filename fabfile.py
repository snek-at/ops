...
def deploy_to_heroku(c, app_instance, local_branch="master", remote_branch="master"):
    check_if_logged_in_to_heroku(c)
    print(
    ...
    local(
        "git push {remote} {local_branch}:{remote_branch}".format(
            remote=remote_name, local_branch=local_branch, remote_branch=remote_branch,
        )
    )
...
def pull_database_from_dokku(c, dokku_remote, app_db_instance):
    ...
    local(
        "ssh {remote} postgres:export {db_instance} | "
...
def push_database_to_dokku(c, dokku_remote, app_instance, db_instance):
    prompt_msg = (
        "You are about to push your local database to Dokku. "
        "It's a destructive operation and will override the "
        "database on the server. \n"
    ...
    if input(prompt_msg) != db_instance:
        raise Exit("Aborted")
    local("ssh {remote} ps:stop {app}".format(remote=dokku_remote, app=app_instance))
    ...
    local(
        ...
        "|| true".format(
            local_db=LOCAL_DATABASE_NAME, remote=dokku_remote, db_instance=db_instance,
        )
    )
    local("ssh {remote} ps:start {app}".format(remote=dokku_remote, app=app_instance))


def pull_media_from_s3_dokku(c, dokku_remote, app_instance):
    aws_access_key_id = get_dokku_variable(
        c, dokku_remote, app_instance, "AWS_ACCESS_KEY_ID"
    )
    aws_secret_access_key = get_dokku_variable(
        c, dokku_remote, app_instance, "AWS_SECRET_ACCESS_KEY"
    )
    aws_storage_bucket_name = get_dokku_variable(
        c, dokku_remote, app_instance, "AWS_STORAGE_BUCKET_NAME"
    )
    pull_media_from_s3(
        c, aws_access_key_id, aws_secret_access_key, aws_storage_bucket_name
    )
...
def get_dokku_variable(c, dokku_remote, app_instance, variable):
    return local(
        "ssh {remote} config:get {app} {var}".format(
            remote=dokku_remote, app=app_instance, var=variable,
        )
    ).stdout.strip()


def deploy_to_dokku(
    c, dokku_remote, app_instance, local_branch="master", remote_branch=None
):
    ...
    deploy_prompt(c, app_instance)
    local(
        "git push {remote}:{app} {local_branch}:{remote_branch}".format(
            remote=dokku_remote,
            app=app_instance,
...
def open_dokku_shell(c, dokku_remote, app_instance):
    local(
        "ssh -t {remote} enter {app}".format(remote=dokku_remote, app=app_instance),
        pty=True,
    )
...
def clean_dokku_database(c, dokku_remote, db_instance):
    local(
        ...
        "{db_instance}-backup.pg".format(remote=dokku_remote, db_instance=db_instance,)
    )
    local(
        'echo "DROP SCHEMA public CASCADE; CREATE SCHEMA public;" | '
        "ssh -t {remote} postgres:connect {db_instance}".format(
            remote=dokku_remote, db_instance=db_instance,
        )
    )
...
def deploy_prompt(c, app_instance):
    prompt_msg = (
        "You are about to do a manual deployment. You probably "
